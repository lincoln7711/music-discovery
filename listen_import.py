...
def cmd_fetch_lastfm() -> None:
    cfg = load_config()
    network = lastfm_network(cfg)
    username = cfg["lastfm"]["username"]

    user = network.get_user(username)
    total = user.get_playcount()
    console.print(f"[bold]Last.fm:[/bold] {username} — {total:,} scrobbles, fetching all pages…\n")
    ...
    with Progress(
        ...
        pages_total = (total // limit) + 2
        task = progress.add_task("Fetching Last.fm pages…", total=pages_total)

        while True:
            try:
                tracks = user.get_recent_tracks(limit=limit, page=page)
            ...
            for t in tracks:
                ...
                scrobbles.append({
                    ...
                    "artist":    t.track.artist.name if t.track.artist else "",
                    "title":     t.track.title,
                    "album":     t.album or "",
                })
...
def cmd_submit() -> None:
    ...
    # Only submit listens not already in Last.fm (within ±60s)
    to_submit = [
        a for a in amazon
        if not any(abs(a["timestamp"] - t) < 60 for t in lfm_timestamps)
    ]
